...
Выберите нужный инструмент ниже.`,
    ...
    aiResult: {
        ...
        midjourneyActionsHint:
            '🖼 Нажмите #1, #2, #3 или #4 — выберите один кадр из сетки.',
        midjourneySingleActionsHint:
            '🖼 Можно сдвинуть кадр, отдалить или перерисовать область.',
        sunoActionsHint:
            '🎵 Выберите трек и действие: Extend, Cover, Vocals или Stems.',
        midjourneyInpaintPrompt:
            '🎨 <b>Перерисовать область</b>: отправьте чёрно-белую маску (белое = перерисовать) и при желании подпись с новым описанием.',
        sunoExtendPrompt:
...
<a href="${SUPPORT_DOCUMENT_URLS.refundPolicy}">Открыть документ</a>`,
    ...
    tools: {
        ...
        instructions: {
            ...
            [AiToolId.CLAUDE_SONNET]: 'Отправьте текст, фото или файл.',
            [AiToolId.GPT_IMAGES]:
            ...
            [AiToolId.FLUX]:
                'Flux 2 Pro: генерация и редактирование по промпту (до 8 референсов). Одно фото без текста — улучшение резкости. Человек + одежда — virtual try-on. В параметрах можно расширить кадр (outpaint).',
    ...
    videoTool: {
        ...
        soraNeedCharacterVideo:
            'Отправьте короткое видео персонажа (2–4 сек). Нельзя лица людей и известных персонажей из фильмов/игр.',
